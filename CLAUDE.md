...
`src/lib/yjs/allotment-yjs.ts` maps `AllotmentData` onto a Yjs document via SyncedStore (`@syncedstore/core`). It exports `createAllotmentDoc`, `hydrateFromJson`, `serializeToJson`, `encodeDocState` / `decodeDocState` for the BYTEA round-trip, `clearAllotmentStore` (empty every container — used by hydrate and by the Step 4 adopt path), and the shared write-site helpers `withoutUndefined` / `assignDefined` used by the domain hooks. This is the canonical local storage engine: `useYjsDoc` (`src/hooks/useYjsDoc.ts`) owns the `Y.Doc`, the `y-indexeddb` persistence provider, and the published `AllotmentData` snapshot; `useAllotmentData` composes it with `useCloudSync`. Domain-hook mutations write through `mutate(fn)` against the SyncedStore proxy. For the Step 4 cloud transport, `useYjsDoc` also exposes `encodeState` (encode the doc for a push), `mergeRemoteUpdate` (CRDT-merge a remote update), `adoptRemoteUpdate` (clear local + apply remote — first-sync lineage adoption), `getSnapshot` (live snapshot on demand), and `hasUpdatesBeyond` (skip a redundant push on a pure pull).
