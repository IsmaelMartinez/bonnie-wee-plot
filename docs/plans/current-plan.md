...
### ADR 027 Yjs Step 5 — Cleanup shipped

The soak window passed with no data anomalies, so Step 5 deleted the legacy storage chain and its rollback scaffolding. `useAllotmentData` now composes `useYjsDoc` (canonical local engine, IndexedDB via `y-indexeddb`) with a new `useCloudSync` hook unconditionally — the `USE_YJS_STORAGE` flag is gone. The eight domain hooks (`useAllotmentAreas`, `useAllotmentPlantings`, `useAllotmentVarieties`, `useAllotmentCustomTasks`, `useAllotmentMaintenance`, `useAllotmentNotes`, `useAllotmentCareLogs`, `useCompost`) keep only their `mutate()` branch; the `setData` prop and legacy branches are removed.

Deleted: `useSyncedStorage`, `usePersistedStorage`, `useYjsToLegacyMirror`, `StorageFlagReloadBanner`, the `bwp-storage-flag` BroadcastChannel, the legacy `allotment-unified-data` debounced-save write path, and the same-tab broadcast apparatus from PR #369 (`bonnie:storage-update` CustomEvent, `instanceId`/`sameTabSeq` bookkeeping, `recordSavedState`/`recordAdoptedState`, `recentSavesRef` echo dedup). The `allotment-unified-data` key is still *read* on first run and *written* by import/restore/fresh-init as the hydrate seed; only the mirror/auto-save write path is gone. Cross-tab sync now rides `y-indexeddb`'s IndexedDB broadcast.

**Cloud sync preserved.** `useCloudSync` (`src/hooks/useCloudSync.ts`) ports the Supabase fetch/push/LWW-guard/conflict-dialog layer, the 30s push debounce, and the unload flush off `usePersistedStorage` to consume the Yjs snapshot directly. Push fires on Yjs snapshot-reference changes (the old chain used `saveStatus === 'saved'`); `'cloud'` conflict resolution adopts the remote via `useYjsDoc.replaceFromJson`, `'local'` pushes the current snapshot. A `normalizedContentSnapshot` round-trips the adopted remote through hydrate/serialize so the republished snapshot isn't pushed straight back. `serializeToJson`/`decodeDocState` stay forever (rollback + GDPR export + debug).

Follow-ups folded in: `src/lib/yjs-spike/` → `src/lib/yjs/` (imports, CLAUDE.md, ADR 027 updated); `yjs-helpers.ts` (`withoutUndefined`/`assignDefined`) consolidated into `allotment-yjs.ts`; the `addInitScript` Playwright seeds (homepage/onboarding) clear Yjs IndexedDB on first load via a new `clearYjsIdbOnFirstLoad` helper in `tests/utils/storage.ts`; the path-parity test became a Yjs-path regression test (`allotment-yjs-storage.test.ts`), and `useSyncedStorage.test.ts` became `useCloudSync.test.ts`. Step 4 (Yjs binary as the cloud transport, Supabase BYTEA) remains the next deliberate work.

### Superseded: Phase 1 soak then Step 5 cleanup
