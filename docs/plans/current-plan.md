...
Last updated: 2026-05-18 (AI/Settings UX sprint shipped in PRs #366–#371; ADR 027 Yjs Step 3 hook integration is the next deliberate work and will delete the same-tab broadcast added in PR #369)
...
Architectural payoff also worth flagging here: a `Y.Doc` is inherently shared state, so the same-tab broadcast apparatus added in PR #369 (the `bonnie:storage-update` CustomEvent, the `instanceId` / `sameTabSeq` bookkeeping, the `recordSavedState` / `recordAdoptedState` helpers, and the `recentSavesRef` echo dedup) all become deletable the day Step 3 lands. Both the implementing agent and the orchestrator surfaced this independently; the broadcast is the cheap holding pattern that keeps the app functional until Yjs replaces the persistence layer. The alternative — lifting `AllotmentData` into a React Context as an intermediate step — would be throwaway work for the same reason, so the recommendation is to go straight to Step 3 rather than land a Context refactor in between.
