...
Last updated: 2026-05-19 (ADR 027 Yjs Step 3 Phase 1 cutover started — PR-B ported the seven domain hooks, PR-B.2 ported useCompost, PR-C flipped `USE_YJS_STORAGE` to default-on; soak window in progress, Step 5 cleanup is the next deliberate work)
...
### Up Next: ADR 027 Yjs Step 3 Phase 1 cutover — soak then Step 5 cleanup

PR-C (this PR) flips `USE_YJS_STORAGE` to default-on. The Yjs path becomes canonical; the legacy chain stays in tree as the rollback floor and the cloud-sync mirror target throughout the soak window. The deployment runbook step that needs running once at deploy time: a no-op `UPDATE allotments SET data = data` against every active row, which triggers the existing `BEFORE UPDATE` history trigger from PR #332 and seeds one pre-cutover snapshot per user into `allotment_history`. Rollback during soak is a flag flip back to `false`, no data migration — the legacy chain has been mirroring throughout. Success criterion for moving to Step 5 cleanup is qualitative for the current two-user cohort: both real users use the app on the flag for ~3–5 days, run a manual cross-device conflict at least once (edit on phone and laptop, watch the conflict-replace path), and report no data anomalies. Once that lands, Step 5 deletes `useSyncedStorage`, the legacy branch of every domain-hook method, `useYjsToLegacyMirror`, the `bwp-storage-flag` BroadcastChannel, the legacy `allotment-unified-data` localStorage key, and the same-tab broadcast apparatus introduced by PR #369 (`bonnie:storage-update` CustomEvent, `instanceId`/`sameTabSeq` bookkeeping, the `recordSavedState`/`recordAdoptedState` helpers, the `recentSavesRef` echo dedup) — every line of that broadcast becomes redundant the day the legacy chain leaves the tree. `serializeToJson` and `decodeDocState` stay forever (rollback + GDPR export + debug).
