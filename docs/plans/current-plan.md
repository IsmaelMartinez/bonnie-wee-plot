...
Last updated: 2026-05-18 (ADR 027 Yjs Step 3 foundation shipped in PRs #382 + #383 — `useYjsDoc` + mirror adapter behind `USE_YJS_STORAGE` default-off; PR-B ports the seven domain-hook files next)
...
### Up Next: ADR 027 Yjs Step 3 PR-B — port the seven domain hooks

PR-B ports `useAllotmentAreas`, `useAllotmentPlantings`, `useAllotmentVarieties`, `useAllotmentCustomTasks`, `useAllotmentMaintenance`, `useAllotmentNotes`, and `useAllotmentCareLogs` to two-branch methods: the legacy branch is the existing `setData(prev => ...)` call unchanged, the Yjs branch calls `mutate(store => { ... in-place mutations on the SyncedStore proxy ... })`. 99 `setData` call sites total, ~10 of which are cross-collection or otherwise non-trivial (`addPlanting` / `addPlantings` touch both plantings and varieties — the spec budgets those at ~10 lines each, the rest at ~3-5 lines). Total surface roughly 600 lines of new Yjs branches plus a single `allotment-path-parity` integration test that scripts a representative mutation sequence through `useAllotment` end-to-end under both flag states and asserts snapshot equality at each step. After PR-B merges, PR-C is a one-line flip of `USE_YJS_STORAGE` to default-on plus a deployment runbook step that runs a no-op `UPDATE allotments SET data = data` so each active user gets one pre-cutover history row in the existing `allotment_history` table.

Architectural payoff worth flagging here: a `Y.Doc` is inherently shared state, so the same-tab broadcast apparatus added in PR #369 (the `bonnie:storage-update` CustomEvent, the `instanceId` / `sameTabSeq` bookkeeping, the `recordSavedState` / `recordAdoptedState` helpers, and the `recentSavesRef` echo dedup) becomes deletable the day PR-C lands. The broadcast is the cheap holding pattern that keeps the app functional until Yjs replaces the persistence layer; lifting `AllotmentData` into a React Context as an intermediate step would be throwaway work for the same reason.
