...
- Deleted: `useSyncedStorage`, `usePersistedStorage`, `useYjsToLegacyMirror`, `StorageFlagReloadBanner`, the `bwp-storage-flag` BroadcastChannel, the legacy `allotment-unified-data` debounced-save write path, and the same-tab broadcast apparatus from PR #369 (`bonnie:storage-update` CustomEvent, `instanceId` / `sameTabSeq` bookkeeping, `recordSavedState` / `recordAdoptedState`, and the `recentSavesRef` echo dedup). The `allotment-unified-data` key is still *read* on first run (and *written* by import / restore / fresh-init) as the hydrate seed; only the mirror/auto-save write path is gone. Cross-tab sync now rides `y-indexeddb`'s IndexedDB broadcast.
