...
export function migrateSchema(data: AllotmentData): AllotmentData {
  ...
  // values (germinated, thinned, flowering, pest, disease, bolted, damage;
  // free-form notes reuse the existing 'observation' type), CareLogEntry.severity
  // / photoId / plantingId, and Planting.endedOn. No data transform needed —
  // every new field is optional and the new care-log types only appear on
  // entries the user creates going forward.
