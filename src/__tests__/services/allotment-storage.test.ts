...
describe('removeSeason', () => {
  ...
  it('cleans up seedsByYear entries from varieties when removing a season', () => {
    const data = createValidAllotmentData({
      currentYear: TEST_CURRENT_YEAR,
      seasons: [
        { year: TEST_CURRENT_YEAR, status: 'current', areas: [], createdAt: '', updatedAt: '' },
      ...
      varieties: [
        {
          ...
          name: 'Test Pea',
          seedsByYear: {
            [TEST_CURRENT_YEAR]: 'have',
            [TEST_CURRENT_YEAR - 1]: 'had',
          },
        }
      ]
    })
    ...
    expect(result.varieties[0].seedsByYear).toEqual({ [TEST_CURRENT_YEAR]: 'have' })
    expect(result.varieties[0].seedsByYear[TEST_CURRENT_YEAR - 1]).toBeUndefined()
  })

  it('does not modify varieties without seedsByYear entries for the deleted year', () => {
    const data = createValidAllotmentData({
      currentYear: TEST_CURRENT_YEAR,
      seasons: [
        { year: TEST_CURRENT_YEAR, status: 'current', areas: [], createdAt: '', updatedAt: '' },
  ...
  it('handles varieties with empty seedsByYear', () => {
    const data = createValidAllotmentData({
      currentYear: TEST_CURRENT_YEAR,
      seasons: [
        { year: TEST_CURRENT_YEAR, status: 'current', areas: [], createdAt: '', updatedAt: '' },
    ...
    expect(result.varieties[0].seedsByYear).toEqual({})
  })

  it('handles data with no varieties', () => {
    const data = createValidAllotmentData({
      currentYear: TEST_CURRENT_YEAR,
      seasons: [
        { year: TEST_CURRENT_YEAR, status: 'current', areas: [], createdAt: '', updatedAt: '' },
